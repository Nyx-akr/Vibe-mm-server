...
const BLUESKY_QUERIES = (process.env.BLUESKY_QUERIES ||
  "memecoin,solana,altcoin,pumpfun"
).split(",").map((x) => x.trim()).filter(Boolean);
...
/**
 * The AT Protocol AppView serves search without a token. Some datacentre
 * ranges are blocked outright (403 with an HTML body), which is reported as a
 * source error rather than silently counted as zero chatter.
 */
function fetchBlueskyPosts() {
  return cached("bluesky:search", SOCIAL_TTL_MS, async () => {
    const collected = [];
    let lastError = null;
    for (const q of BLUESKY_QUERIES) {
      try {
        const d = await fetchJson(BLUESKY_SEARCH + "?q=" + encodeURIComponent(q) + "&limit=50&sort=latest");
        for (const p of (d && d.posts) || []) {
          collected.push(post(
            "bluesky", p.uri,
            stripHtml(((p.record || {}).text) || ""),
            (p.author && p.author.handle) || null,
            p.replyCount || 0,
            (p.likeCount || 0) + (p.repostCount || 0),
            toTimestampMs((p.record || {}).createdAt || p.indexedAt),
          ));
        }
...
const SOCIAL_FEEDS = [
  ...
  { source: "bluesky", label: "bluesky appview post search", fetch: fetchBlueskyPosts },
];
