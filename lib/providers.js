...
const { fetchJson, fetchText, cached, toNumber, toTimestampMs } = require("./fetcher");
...
/**
 * Public social feeds, in one shape.
 *
 * Every source here answers without an API key. X/Twitter is deliberately
 * absent: api.twitter.com/2 answers 401 to every keyless request and the
 * cheapest read tier is paid, so there is no honest free path to it. The
 * DexScreener promotion feed still surfaces the x.com LINK a token advertises,
 * which is a fact about the token, not a measurement of X chatter.
 *
 * Each source is fetched and cached independently so one outage degrades the
 * scanner instead of emptying it. Every post leaves this file as:
 *
 *   { source, id, text, author, replies, reactions, time }
 *
 * and nothing else happens to it. Symbol matching, mention counts, unique
 * author counts and baselines are the app's job.
 */
const SOCIAL_RECENT_MS = Number(process.env.SOCIAL_RECENT_MS || 172800000);

const BIZ_CATALOG = "https://a.4cdn.org/biz/catalog.json";
const BIZ_THREAD = "https://a.4cdn.org/biz/thread/";
const BIZ_THREAD_EXPAND = Number(process.env.BIZ_THREAD_EXPAND || 12);

const REDDIT_SUBS = (process.env.REDDIT_SUBS ||
  "CryptoMoonShots,solana,CryptoCurrency,memecoins,altcoin,CryptoCurrencyTrading,ethtrader"
).split(",").map((x) => x.trim()).filter(Boolean);

const MASTODON_HOST = process.env.MASTODON_HOST || "https://mastodon.social";
const MASTODON_TAGS = (process.env.MASTODON_TAGS ||
  "crypto,solana,memecoin,altcoin,defi,cryptocurrency"
).split(",").map((x) => x.trim()).filter(Boolean);

const FARCASTER_SEARCH = "https://api.warpcast.com/v2/search-casts";
const FARCASTER_QUERIES = (process.env.FARCASTER_QUERIES ||
  "memecoin,solana,pumpfun,altcoin"
).split(",").map((x) => x.trim()).filter(Boolean);
...
/**
 * Tags out, entities decoded, whitespace collapsed. Parsing, not maths.
 *
 * Two passes, because Reddit's Atom feed escapes an HTML document inside an
 * HTML element: decoding the entities once only reveals a second layer of
 * real tags (a div, an SC_OFF comment) that still has to come off.
 */
...
function post(source, id, text, author, replies, reactions, time) {
  return {
    source: source,
    id: String(id),
    text: text,
...
/**
 * When each feed's bytes were last actually fetched.
 *
 * `cached` only runs a producer on a miss, so writing the stamp inside the
 * producer gives the real age of the corpus rather than the age of the last
 * request. The app polls far faster than these feeds change; this is what lets
 * it ask "anything new?" instead of re-downloading a megabyte of posts it
 * already has.
 */
const feedFetchedAt = Object.create(null);

function stampFeed(source) { feedFetchedAt[source] = Date.now(); return true; }

/** Newest fetch across all feeds - the corpus version, in effect. */
function socialCorpusAt() {
  let newest = 0;
  for (const key of Object.keys(feedFetchedAt)) {
    if (feedFetchedAt[key] > newest) newest = feedFetchedAt[key];
  }
  return newest;
}
...
/**
 * The catalog is only the opening post of each thread - about 200 posts for
 * the whole board, which is why ticker matching used to find nothing. The
 * busiest threads are expanded into their replies, where tickers actually
 * get named.
 */
function fetchBizThreads() {
  return cached("biz:catalog", SOCIAL_TTL_MS, async () => {
    ...
    const ops = threads.map((t) => post(
      "4chan", t.no,
      stripHtml((t.sub || "") + " " + (t.com || "")),
      null, t.replies || 0, 0, (t.time || 0) * 1000,
    ));

    const busiest = threads.slice()
      .sort((a, b) => (b.replies || 0) - (a.replies || 0))
      .slice(0, BIZ_THREAD_EXPAND);

    const replies = [];
    for (const thread of busiest) {
      try {
        const body = await fetchJson(BIZ_THREAD + thread.no + ".json");
        for (const p of (body && body.posts) || []) {
          if (p.no === thread.no) continue;
          replies.push(post(
            "4chan", p.no,
            stripHtml((p.sub || "") + " " + (p.com || "")),
            null, 0, 0, (p.time || 0) * 1000,
          ));
        }
      } catch (error) { /* one dead thread must not empty the board */ }
      await sleep(120);
    }
    return ops.concat(replies).filter((p) => p.text);
  });
}
...
/**
 * Reddit's JSON endpoints answer 403 to datacentre IPs; the Atom feed of the
 * same listing answers 200, so the feed is what we read.
 *
 * Reddit throttles this hard and per-IP - a burst of seven subreddits earns a
 * 429 that then applies to all of them. So only REDDIT_BATCH of them are read
 * per refresh, spaced out, and the window walks along the list on each refresh
 * so the whole set is still covered over time at a fraction of the request
 * rate. The feed is also cached longer than the others for the same reason.
 */
const REDDIT_TTL_MS = Number(process.env.REDDIT_TTL_MS || 900000);
const REDDIT_BATCH = Number(process.env.REDDIT_BATCH || 3);
const REDDIT_GAP_MS = Number(process.env.REDDIT_GAP_MS || 1500);

/* Reddit answers 429 to unrecognised agents far more readily than to a browser. */
const REDDIT_UA =
...
function parseRedditAtom(xml, sub) {
  ...
  return entries.map((entry, index) => {
    ...
    const authorBlock = (entry.split("<author>")[1] || "").split("</author>")[0];
    const authorName = (authorBlock.match(/<name>([\s\S]*?)<\/name>/) || [])[1];
    const time = Date.parse(stripHtml(pick("updated")) || stripHtml(pick("published")));
    return post(
      "reddit",
      stripHtml(pick("id")) || sub + ":" + index,
      stripHtml(pick("title")) + " " + stripHtml(pick("content")),
      stripHtml(authorName) || null,
      0, 0,
...
function fetchRedditPosts() {
  return cached("reddit:new", REDDIT_TTL_MS, async () => {
    stampFeed("reddit");
    const batch = [];
    for (let i = 0; i < Math.min(REDDIT_BATCH, REDDIT_SUBS.length); i++) {
      batch.push(REDDIT_SUBS[(redditCursor + i) % REDDIT_SUBS.length]);
    }
    redditCursor = (redditCursor + batch.length) % REDDIT_SUBS.length;

    const out = [];
    const failures = [];
    for (const sub of batch) {
      const url = "https://www.reddit.com/r/" + sub + "/new/.rss?limit=40";
      try {
        const xml = await fetchText(url, { headers: { "user-agent": REDDIT_UA } });
        out.push.apply(out, parseRedditAtom(xml, sub));
      } catch (error) { failures.push(sub + ": " + error.message); }
      await sleep(REDDIT_GAP_MS);
    }
    if (!out.length && failures.length) throw new Error(failures.join("; ").slice(0, 160));
    return out;
  });
}
...
/** Public hashtag timelines. No key, no auth, no rate ceiling worth gating. */
function fetchMastodonPosts() {
  return cached("mastodon:tags", SOCIAL_TTL_MS, async () => {
    stampFeed("mastodon");
    const results = await Promise.all(MASTODON_TAGS.map((tag) =>
      fetchJson(MASTODON_HOST + "/api/v1/timelines/tag/" + encodeURIComponent(tag) + "?limit=40")
        .then((rows) => (Array.isArray(rows) ? rows : []).map((s) => post(
          "mastodon", s.id,
          stripHtml(s.content),
          (s.account && s.account.acct) || null,
          s.replies_count || 0,
          (s.favourites_count || 0) + (s.reblogs_count || 0),
          toTimestampMs(s.created_at),
        )))
        .catch(() => null)));
    const ok = results.filter(Boolean);
    if (!ok.length) throw new Error("every mastodon tag timeline failed");
    return ok.flat().filter((p) => p.text);
  });
}
...
/**
 * Warpcast's keyless search ranks by relevance, not recency, so it returns
 * casts years old beside today's. Anything outside the recency window is
 * dropped here rather than being passed off as current chatter.
 */
function fetchFarcasterCasts() {
  return cached("farcaster:search", SOCIAL_TTL_MS, async () => {
    stampFeed("farcaster");
    const cutoff = Date.now() - SOCIAL_RECENT_MS;
    const results = await Promise.all(FARCASTER_QUERIES.map((q) =>
      fetchJson(FARCASTER_SEARCH + "?q=" + encodeURIComponent(q) + "&limit=50")
        .then((d) => (((d && d.result) || {}).casts || []).map((c) => post(
          "farcaster", c.hash,
          stripHtml(c.text),
          (c.author && c.author.username) || null,
          (c.replies && c.replies.count) || 0,
          (c.reactions && c.reactions.count) || 0,
          toTimestampMs(c.timestamp),
        )))
        .catch(() => null)));
    const ok = results.filter(Boolean);
    if (!ok.length) throw new Error("warpcast search unavailable");
    return ok.flat().filter((p) => p.text && p.time && p.time >= cutoff);
  });
}
...
/**
 * Bluesky, the closest public stand-in for X.
 *
 * app.bsky.feed.searchPosts now answers 403 to unauthenticated callers, so
 * search is not a route. The rest of the AppView is still open: the popular
 * feed generators can be discovered by query, and getFeed serves any of them
 * without a token. Reading the community's own crypto feeds is a better
 * sample than a keyword search anyway - somebody curates them.
 */
const BSKY_BASE = "https://public.api.bsky.app/xrpc";
const BSKY_FEED_QUERIES = (process.env.BLUESKY_FEED_QUERIES || "crypto,solana"
).split(",").map((x) => x.trim()).filter(Boolean);
const BSKY_FEEDS_PER_QUERY = Number(process.env.BLUESKY_FEEDS_PER_QUERY || 2);
const BSKY_FEED_LIMIT = Number(process.env.BLUESKY_FEED_LIMIT || 60);
/** Feed generators change rarely; their contents do not. */
const BSKY_DISCOVER_TTL_MS = Number(process.env.BLUESKY_DISCOVER_TTL_MS || 86400000);

/** The feed discovery settled on, kept so the UI can link to the real thing. */
let lastBlueskyFeedUri = null;

/** at://did:plc:xxx/app.bsky.feed.generator/yyy -> the bsky.app page for it. */
function blueskyFeedUrl() {
  const m = String(lastBlueskyFeedUri || "").match(/^at:\/\/([^/]+)\/app\.bsky\.feed\.generator\/(.+)$/);
  return m ? "https://bsky.app/profile/" + m[1] + "/feed/" + m[2] : "https://bsky.app/";
}

function discoverBlueskyFeeds() {
  return cached("bluesky:generators", BSKY_DISCOVER_TTL_MS, async () => {
    const uris = [];
    for (const q of BSKY_FEED_QUERIES) {
      try {
        const d = await fetchJson(BSKY_BASE + "/app.bsky.unspecced.getPopularFeedGenerators" +
          "?query=" + encodeURIComponent(q) + "&limit=10");
        for (const feed of ((d && d.feeds) || []).slice(0, BSKY_FEEDS_PER_QUERY)) {
          if (feed && feed.uri && uris.indexOf(feed.uri) === -1) uris.push(feed.uri);
        }
      } catch (error) { /* one query failing must not lose the others */ }
    }
    if (!uris.length) throw new Error("no bluesky feed generators discovered");
    lastBlueskyFeedUri = uris[0];
    return uris;
  });
}

function fetchBlueskyPosts() {
  return cached("bluesky:feeds", SOCIAL_TTL_MS, async () => {
    stampFeed("bluesky");
    const uris = await discoverBlueskyFeeds();
    const collected = [];
    let lastError = null;
    for (const uri of uris) {
      try {
        const d = await fetchJson(BSKY_BASE + "/app.bsky.feed.getFeed" +
          "?feed=" + encodeURIComponent(uri) + "&limit=" + BSKY_FEED_LIMIT);
        for (const item of (d && d.feed) || []) {
          ...
          collected.push(post(
            ...
            (p.author && p.author.handle) || null,
            p.replyCount || 0,
            (p.likeCount || 0) + (p.repostCount || 0),
            toTimestampMs(record.createdAt || p.indexedAt),
          ));
        }
    ...
    return collected.filter((p) => p.text);
  });
}
...
/**
 * Each feed also carries `url`: the page a person can open to read the same
 * thing this server just read. Built from the same constants the fetchers use,
 * so the link cannot drift from what was actually sampled - change the
 * subreddit list and the link follows.
 */
const SOCIAL_FEEDS = [
  {
    source: "4chan",
    label: "4chan /biz/ catalog + busiest threads",
    url: "https://boards.4chan.org/biz/",
    fetch: fetchBizThreads,
  },
  {
    source: "reddit",
    label: "reddit atom feeds (" + REDDIT_BATCH + " of " + REDDIT_SUBS.length + " subreddits per refresh)",
    url: "https://www.reddit.com/r/" + REDDIT_SUBS.join("+") + "/new/",
    fetch: fetchRedditPosts,
  },
  {
    source: "mastodon",
    label: "mastodon public hashtag timelines",
    url: MASTODON_HOST + "/tags/" + encodeURIComponent(MASTODON_TAGS[0] || "crypto"),
    fetch: fetchMastodonPosts,
  },
  {
    source: "farcaster",
    label: "warpcast keyless cast search",
    url: "https://warpcast.com/~/search/casts?q=" + encodeURIComponent(FARCASTER_QUERIES[0] || "memecoin"),
    fetch: fetchFarcasterCasts,
  },
  {
    source: "bluesky",
    label: "bluesky crypto feed generators",
    // Resolved per request: the feed is discovered, not hard-coded.
    url: blueskyFeedUrl,
    fetch: fetchBlueskyPosts,
  },
];

/**
 * Every feed at once. A source that fails is reported with its error and a
 * zero count; it never takes the others down with it.
 */
async function fetchSocialPosts() {
  const settled = await Promise.all(SOCIAL_FEEDS.map((feed) =>
    feed.fetch()
      .then((rows) => ({ feed: feed, rows: rows || [], error: null }))
      .catch((error) => ({ feed: feed, rows: [], error: String(error.message || error).slice(0, 160) }))));

  return {
    posts: settled.flatMap((s) => s.rows),
    sources: settled.map((s) => ({
      source: s.feed.source,
      label: s.feed.label,
      url: typeof s.feed.url === "function" ? s.feed.url() : s.feed.url,
      fetchedAt: feedFetchedAt[s.feed.source] || null,
      ok: s.error === null,
      error: s.error,
      posts: s.rows.length,
    })),
...
module.exports = {
  ...
  fetchUsdReference, fetchBizThreads, fetchSocialPosts, socialCorpusAt, fetchPromotion,
};
